// Text Adventure Game

// game variables
const start = `Welcome to the Ghostbusters Text Adventure Game. Click 'OK' to start the game.`;

const enter = "Please enter 1 or 2 for your answer";

const gameOver = "Sorry GAME OVER!";

const q1 = `You are trying to catch a ghost called "Slimer" at the Sedgwick Hotel but have not seen him yet. Should you...
1. Take the elevator to look on the 12th floor
OR
2. Wait in the lobby to see if he shows up there
${enter}`;

const gameOver1 = `You never see Slimer so you give up on catching ghosts and become a talk show host instead. ${gameOver}`; // answer 2

const q2 = `When you get off the elevator something startles you! Should you...
1. Look to see what it is
OR
2. Shoot at it immediately with your proton pack
${enter}`; // answer 1

const gameOver2 = `It was the hotel maid and you nearly killed her! You are forced to leave the hotel. ${gameOver}`; // answer 2

const q3 = `It was just the hotel maid. So you walk around to look for Slimer and You Spot Him! Should you...
1. Call to let the other Ghostbusters know you found him
OR
2. Hide and wait to see where he goes
${enter}`; // answer 1

const gameOver3 = `Slimer hears you, immediately flies at you, and YOU GET SLIMED! ${gameOver}`; // answer 1

const q4 = `You see that Slimer goes into the ballroom so you quickly tell the other Ghostbusters and you all head in. You see him flying around when you walk in. Should you...
1. Shoot at him with your proton pack
OR
2. Put two proton packs together to try and create a more powerful shot (A.K.A. Cross The Streams)
${enter}`; // answer 2

const gameOver4 = `All life as you know it stops instantaneously and every molecule in your body explodes at the speed of light. ${gameOver}`; // answer 2

const q5 = `You wrangled Slimer with your shot and the ghost trap is set underneath him. You noticed there is a bright light coming from the trap and are tempted to look at it. Should you...
1. Look at the trap
OR
2. Keep your focus on Slimer
${enter}`; // answer 1

const gameOver5 = `You looked at the trap and the light BLINDED YOU! ${gameOver}`; // answer 1

const win = `The ghost trap sucked Slimer in and closed. CONGRATULATIONS! YOU CAUGHT SLIMER!!!`; // answer 2


// functionality
alert(start); // alerts user to start game

let userInput = prompt(q1); // set user's input to userInput var

// NESTED CONDITIONALS STATEMENTS
// NOTE: prompts always return as a string (why == is used instead of ===)
// question 1   -   1st Conditional Statements
if (userInput == 1){ // correct
    userInput = prompt(q2);
    // question 2   -   2nd Conditional Statements
    if (userInput == 1){ // correct
        userInput = prompt(q3);
        // question 3   -   3rd Conditional Statements
        if (userInput == 1){ // incorrect
            alert(gameOver3);
        }
        else if (userInput == 2){ // correct
            userInput = prompt(q4);
            // question 4   -   4th Conditional Statements
            if (userInput == 1){ // correct
                userInput = prompt(q5);
                // question 5   -   5th Conditional Statements
                if (userInput == 1){ // incorrect
                    alert(gameOver5);
                }
                else if (userInput == 2){ // correct
                    alert(win);
                }
                else { // not an option
                    alert("Not an option. Answer 1 or 2.")
                }
            }
            else if (userInput == 2){ // incorrect
                alert(gameOver4);
            }
            else { // not an option
                alert("Not an option. Answer 1 or 2.")
            }
        }
        else { // not an option
            alert("Not an option. Answer 1 or 2.")
        }
    }
    else if (userInput == 2){ // incorrect
        alert(gameOver2);
    }
    else { // not an option
        alert("Not an option. Answer 1 or 2.")
    }
}
else if (userInput == 2){ // incorrect
    alert(gameOver1);
}
else { // not an option
    alert("Not an option. Answer 1 or 2.")
}

/*
if (userInput == 1){ // correct
    
}
else if (userInput == 2){ // incorrect

}
else { // not an option
    alert("Not an option. Answer 1 or 2.")
}
*/